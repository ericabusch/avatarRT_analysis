...
fig, ax = make_barplot_points(
    ...
    ylabel='Δ Explained Variance Ratio', xlabel='',print_stats=False
)
...
print(f'IM   vs 0:   mean={m_im_0*100:.4f}  p={p_im_0:.4f}    95%CI=[{lower_im_0*100:.4f},{upper_im_0*100:.4f}]    d={d_im_0:.4f}')
print(f'WMP  vs 0:   mean={m_wmp_0*100:.4f} p={p_wmp_0:.4f}   95%CI=[{lower_wmp_0*100:.4f},{upper_wmp_0*100:.4f}]  d={d_wmp_0:.4f}')
print(f'OMP  vs 0:   mean={m_omp_0*100:.4f} p={p_omp_0:.4f}   95%CI=[{lower_omp_0*100:.4f},{upper_omp_0*100:.4f}]  d={d_omp_0:.4f}')
...
print(f"  IM  vs WMP:  mean={m_im_wm*100:.4f}  p={p_im_wm:.4f}  95%CI=[{lo_im_wm*100:.4f},{hi_im_wm*100:.4f}]  d={helper.cohens_d_paired(v_im - v_wm):.4f}")
print(f"  IM  vs OMP:  mean={m_im_om*100:.4f}  p={p_im_om:.4f}  95%CI=[{lo_im_om*100:.4f},{hi_im_om*100:.4f}]  d={helper.cohens_d_paired(v_im - v_om):.4f}")
print(f"  WMP vs OMP:  mean={m_wm_om*100:.4f}  p={p_wm_om:.4f}  95%CI=[{lo_wm_om*100:.4f},{hi_wm_om*100:.4f}]  d={helper.cohens_d_paired(v_wm - v_om):.4f}")
...
me_coefs_df.to_csv(os.path.join(RESULTS_PUBLIC, 'neural_EVR_lm_coefs.csv'), index=False)
simple_slopes_df.to_csv(os.path.join(RESULTS_PUBLIC, 'neural_EVR_simple_slopes.csv'), index=False)
print(f'\nSaved model results to {RESULTS_PUBLIC}')

...
# Exclude subjects excluded from neural analyses
exclude_str = [f'avatarRT_sub_{s:02d}' for s in exclude_from_neural_analyses]
wide = wide[~wide['subject_id'].isin(exclude_str)].reset_index(drop=True)
print(f'After exclusions: {len(wide)} subjects')

# ── Match notebook variable names ─────────────────────────────────────────────
...
# ── Plot: replicate neural_results.ipynb ──────────────────────────────────────
...
for i, (yname, ylabel, yoff) in enumerate(zip(ynames, ylabels, yplus)):
    ...
    _, pv, _ = helper.permutation_test(
        np.array([points0, points1]), n_iterations=10000, alternative='two-sided')
    print(f'{yname}  permutation p = {pv:.4f}')
    if pv > 0.5:
        pv = 1 - pv

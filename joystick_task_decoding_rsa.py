...
_OLD_JOY_FILE  = os.path.join(INTERMEDIATE_RESULTS_PATH, 'joystick_decoding_results_final.csv')
...
for i, (yname, ylabel, yoff) in enumerate(zip(ynames, ylabels, yplus)):
    ...
    _, pv, _ = helper.permutation_test(
        np.array([arr0, arr1]), n_iterations=NPERM, alternative='two-sided')
    m0, lo0, hi0, _       = helper.bootstrap_ci(arr0, n_boot=NBOOT, verbose=0)
    m1, lo1, hi1, _       = helper.bootstrap_ci(arr1, n_boot=NBOOT, verbose=0)
    m_diff, lo_d, hi_d, _ = helper.bootstrap_ci(arr0 - arr1, n_boot=NBOOT, verbose=0)
    ...
    all_stats.append({
        ...
        'test': f'permutation_test (n_iter={NPERM}, alternative=two-sided)',
    ...
    sns.despine()
    plt.tight_layout()
    plt.savefig(os.path.join(PLOTS_PATH, f'joystick_{yname}.pdf'),
...
stats_fn = os.path.join(FINAL_RESULTS_PATH, 'joystick_stats.csv')

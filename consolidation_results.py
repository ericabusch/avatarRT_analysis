# consolidation_results.py
# Analyzes evidence for neural consolidation after learning, as in the
# "new consolidation after learning" section of revision.ipynb.
#
# Analysis 1 — EVR resampling (cells 59-66):
...
#
# Analysis 2 — Cross-session decoding (cells 50-55):
#   IM-trained decoders evaluated across sessions: delta_mse (run 4 - run 1).
#   Requires: results/intermediate_results/decoding_results_jul30_cross_session_run_cross_validation.csv
...
def plot_decoding_results(df_difference):
    ...
    _, lower_omp, upper_omp = helper.bootstrap_ci(a[0], n_boot=10000)
    _, lower_wmp, upper_wmp = helper.bootstrap_ci(b[0], n_boot=10000)
    print(f'OMP delta_mse: p={p_omp:.4f}, 95% CI=[{lower_omp:.4f}, {upper_omp:.4f}]')
    print(f'WMP delta_mse: p={p_wmp:.4f}, 95% CI=[{lower_wmp:.4f}, {upper_wmp:.4f}]')
